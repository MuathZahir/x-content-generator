...
- Generates 3-5 reply options written to read like a real person, with an optional free-text note to steer tone or angle (e.g. "make it sarcastic", "ask about pricing").
- Prompts the model against common AI writing tells (antithesis "not X, it's Y" flips, rule-of-three lists, em dashes, hype words, sentence stacking) and strips em/en dashes from output as a hard guard.
