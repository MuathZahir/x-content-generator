...
- **Discover now shows who posted and how it's landing, and stops sending you into dead threads.** Each suggestion now carries the author (name, @handle, verified badge, avatar), the post's age, and its real engagement — likes, replies, and views — in a redesigned, X-native card. Under the hood, the server enriches the shortlist with each post's live metrics and uses them to drop dead or buried posts (the "0–2 likes, deep in a thread nobody sees" case), spread the list across different authors instead of one prolific account, and rank by genuine traction (on a log scale, so a couple of viral posts don't crowd out smaller, on-point ones). The search and curation prompts now also favor original standalone posts over replies buried in long threads. If a niche product turns up nothing with real reach, it relaxes and shows the best of what was found rather than coming back empty. Enrichment is best-effort — a post that can't be looked up is still shown, just without metrics. This spends more SocialCrawl credits per run (1 search + up to ~12 post lookups), so the daily Discover ceiling was tightened from 30 to **15/day** to keep that cost bounded.

## 1.3.0 - 2026-06-27

### Added

- **Discover: find X posts worth replying to about your products (Pro).** A new Discover tab in the panel searches recent X posts where one of your products is a genuine, helpful answer, then ranks them and explains why each is a real opening and how you might reply. Clicking a post opens it and immediately switches the panel to Assist and starts drafting reply options for that post, so you go from the list to editable drafts in one click. The list persists locally (for an hour) so it survives opening posts and switching pages, and opened posts are marked so you can work through them. It surfaces conversations; it never auto-replies, never keeps a server-side list, and never posts for you. Search runs server-side through the hosted API (the search key lives on the server, like the model key); each search counts as one daily generation and is refunded when nothing useful comes back. Hosted endpoint: `POST /v1/discover`. Works offline in mock mode with sample results.

### Changed

- **Cost-and-abuse hardening of the hosted plans (server-side).** Following a unit-economics audit, the caps were re-layered so no single dimension carries abuse protection alone. The Pro daily allowance moves from 400 to **100 generations a day**, with a new **1,500/month** fair-use backstop and a per-user **daily model-token circuit breaker**. Discover now has its own **never-refunded daily ceiling (15/day for Pro)**: because every Discover attempt spends search credits and a curation call even when it finds nothing, the daily-call refund no longer hands back a free retry — closing a loop where a no-match product could repeatedly search at no quota cost. Discover curation now runs on `gpt-5.4-mini` (it only ranks search results, so the flagship model was unnecessary). Reply and refine generation now run at low reasoning effort, matching the other endpoints, to cut output-token cost on the highest-volume path. Model token usage is now logged per call and accumulated per user per day, with a `high_token_user` warning when an account crosses the alert threshold. No change to the human-in-the-loop behavior, the value ladder, or the price.
